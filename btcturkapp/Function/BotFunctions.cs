    ...
    public class BotFunctions
    {
        public void startBotFunc(Button startBotButton,TextBox botMarjTextBox,ComboBox comboBox1,DataGridView sellBuyBuySellDifferenceGridView,DataGridView abritajGridView,DataGridView priceDataGridView, ListBox listBox1, TextBox btcTurkIdTextBox, TextBox binanceIdTextBox, Button btcTurkEmirIptalButton, Button binanceEmirIptalButton,string symbolForCancelBinance,DataGridView assetGridView)
        {
            ...
            else
            {
                ...
                {
                    var responseBinance = await binance.BinanceGetOrderBookAsync("USDTTRY");
                    var hacimBinance = responseBinance.Bids[0][1].ToString();

                    var responseBtcTurk= await btcTurk.BTCTurkGetOrderBookAsync("USDTTRY");
                    var hacimBtcTurk= responseBtcTurk.Bids[0][1].ToString("0");

                    Console.WriteLine(double.Parse(hacimBtcTurk));

                    if (botMarjTextBox.Text != "" && comboBox1.SelectedIndex != -1)
                    {
                        if (secim == 1 && marj >= double.Parse(botMarjTextBox.Text) && double.Parse(hacimBtcTurk) < 5000 && double.Parse(assetGridView.Rows[0].Cells[1].Value.ToString()) >= double.Parse(abritajGridView.Rows[0].Cells[2].Value.ToString()))
                        {
                        ...
                        else if (secim == 2 && marj >= double.Parse(botMarjTextBox.Text) && double.Parse(hacimBtcTurk) < 5000 && double.Parse(assetGridView.Rows[0].Cells[1].Value.ToString()) >= double.Parse(abritajGridView.Rows[1].Cells[2].Value.ToString()))
                        {
                        ...
                        else if (secim == 3 && marj >= double.Parse(botMarjTextBox.Text) && double.Parse(hacimBtcTurk) < 5000 && double.Parse(assetGridView.Rows[0].Cells[3].Value.ToString()) >= double.Parse(abritajGridView.Rows[2].Cells[2].Value.ToString()))
                        {
                        ...
                        else if (secim == 4 && marj >= double.Parse(botMarjTextBox.Text) && double.Parse(hacimBtcTurk) < 5000 && double.Parse(assetGridView.Rows[0].Cells[3].Value.ToString()) >= double.Parse(abritajGridView.Rows[3].Cells[2].Value.ToString()))
                        {
