...
namespace TradingBot
{
	public class API
	{
		...
		//public async static bool Ping()
		//{
		//	string apiRequestUrl = $"{baseUrl}v1/ping";

		//	string response = await Helper.webRequest(apiRequestUrl, "GET", null);
		//	if (response == "{}")
		//		return true;
		//	else
		//		return false;
  ...
  //      internal static object GetOnePrice()
  //      {
  ...
  //      }
        ...
        public  SymbolTicker[] GetAllPrices()
		{
			string apiRequestUrl = $"{baseUrl}v3/ticker/bookTicker";

			var response =  Helper.webRequest(apiRequestUrl, "GET", null);
			var parsedResponse = JsonConvert.DeserializeObject<SymbolTicker[]>(response);

			return parsedResponse;
		}

		public string GetOnePrice(string symbol)
		{			
			string apiRequestUrl = $"{baseUrl}v3/ticker/bookTicker?symbol=" + symbol;
			var response =  Helper.webRequest(apiRequestUrl, "GET", null);
			var parsedResponse = JsonConvert.DeserializeObject<SymbolTicker[]>(response);

			SymbolTicker symbolPrice = (from sym in parsedResponse where sym.Symbol == symbol select sym).FirstOrDefault();
			if (symbolPrice == null)
			{
				throw new ApplicationException($"No symbol, {symbol}, exists");
			}

			return "Bid: " + symbolPrice.BidPrice + " \n\n" + "Ask: " + symbolPrice.AskPrice + " ";
		}

        //public Ticker24hr[] GetAllPrices24Hr()
        //{
        //	string apiRequestUrl = $"{baseUrl}v3/ticker/24hr";

        //	var response = Helper.webRequest(apiRequestUrl, "GET", null);
        //	var parsedResponse = JsonConvert.DeserializeObject<Ticker24hr[]>(response);

        //	return parsedResponse;
        //}


        //public AssetBalance[] GetAllAssets()
        //{
        //	string apiRequestUrl = $"{baseUrl}v3/account";

        //	var response = Helper.webRequest(apiRequestUrl, "GET", null);
        //	var parsedResponse = JsonConvert.DeserializeObject<AssetBalance[]>(response);

        //	return parsedResponse;
        //}

        //public double GetOnePrices24Hr(string symbol)
        //{
        //	var symbolPrices = GetAllPrices24Hr();

        //	Ticker24hr symbolPrice = (from sym in symbolPrices where sym.Symbol == symbol select sym).FirstOrDefault();
        //	if (symbolPrice == null)
        //	{
        //		throw new ApplicationException($"No symbol, {symbol}, exists");
        //	}

        //	return symbolPrice.BidPrice;
        //}
        ...
        public static Order PlaceMarketOrder(string symbol, OrderSides side, double quantity)
        {
            ...
            string query = $"symbol=BTCUSDT&side=SELL&type=LIMIT&timeInForce=GTC&quantity=0.001&price=44000";
            query = $"{query}&timestamp={Helper.getTimeStamp()}";
